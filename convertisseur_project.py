############################################################## importation des éléments utilisés
from tkinter import *
from tkinter import ttk
################################################## on défini une fenêtre
root = Tk()
root.title('Convertisseur Monétaire')
############################### On rajoute quelques élements à la fenêtre comme des Boutons, des Listes déroulantes
frame1 = Frame(root, padx = 60, pady = 30)
frame1.pack()
label = Label(frame1, text = "Convertisseur Monétaire", font = 'TimesNewRoman 24', fg = 'red', bd = 2)
label.pack(side = TOP)
...
############################### Liste déroulante 1 pour choisir nos devises
...
def convertir(t1,t2):
    ...
    if len(str(nombre2)) > 0 and len(str(nombre1)) > 0:
        ...
        f.close()
def appel_convertir():
    global deviseOne ####################### Le global permet d'indiquer que c'est une variable vue juste avant
    global deviseTwo
    global devise
    select1 = devise.get()
    select2 = devise2.get()
############################## définition des conditions pour la conversion
